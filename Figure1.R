...
#add ellipses
fig1 <- ordination_plot +
  stat_ellipse(geom = "polygon", type="norm", alpha=0.4, aes(fill=sample_type))

#save ordination plot 
ggsave("ordinationplot_developmentsages.png", width = 5, height = 5)
...
uw_adonis <- adonis2(distance(combined_subset2, method="unifrac") ~ sample_type, data = md_combined_subset, permutations = 9999)
uw_adonis
...
fig2 <- df_Genus %>%
  ...
  facet_nested(~ sample_type + Cell_ID + sampleid, scales = "free", space = "free") +
...
Figure1 <- cowplot::plot_grid(fig2,
                             fig1)

ggsave("Figure1.png", height=15, width=20)
